...
const CLOJURE_REPL_CONTROL_PROMPT = [
	...
	'`(process-start "cmd")` runs a shell command and returns immediately with a handle map holding `:process-id`; it does not wait for the command. `(process-poll id)` reports `:status` (`:running` or `:exited`), `:exit-code`, and whether output was truncated. `(process-tail id)` returns the last 50 captured lines, `(process-tail id n)` the last n. `(process-kill id)` terminates the command and its children. `(process-list)` shows the registry. Pass the `:process-id` string or the handle map itself.',
