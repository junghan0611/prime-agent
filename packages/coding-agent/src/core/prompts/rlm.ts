...
const CLOJURE_REPL_CONTROL_PROMPT = [
	...
	'`(process-start "cmd")` runs a shell command and returns immediately with a handle map holding `:process-id`; it does not wait for the command. `(process-poll id)` reports `:status` (`:running` or `:exited`), `:exit-code`, and whether output was truncated. `(process-tail id)` returns the last 50 captured lines, `(process-tail id n)` the last n. `(process-kill id)` terminates the whole process group of the command. `(process-list)` shows the registry. Pass the `:process-id` string or the handle map itself.',
	"The process is never a value in this workspace, captured output is capped head-and-tail, and there is no sleep to block on. To wait for a command, poll it in a later cell instead of looping on `process-poll` inside one cell.",
	"",
