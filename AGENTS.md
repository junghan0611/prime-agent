...
1. 이것이 **비교 검출**인가, 코드 품질에 대한 의견인가?
2. 이 문장에 **영수증**이 붙는가, 아니면 인상인가?
3. 지금 뚫는 단계인가 재는 단계인가 — 그리고 그것을 GLG 와 합의했는가?
4. 이 주장이 커버리지 없이 가능성을 말하고 있지 않은가?
...
- **`prime-agent-runtime-clj/`** — Clojure workspace. `rlm-repl` 네이티브 바이너리, verb 는 `read-text`/`write-text`/`edit-text`/`process-*`/`rlm`/`rlm-children`/`host-request`. SCI 샌드박스라 `spit`/`slurp`/interop 은 닫혀 있다.
...
| **native SUT** (`clojure -M:test`) | clojure 동작 vs 적어둔 스펙 | 0 | verb 단위 회귀 |
